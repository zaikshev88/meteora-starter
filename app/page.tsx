...
// ?v=1 or no param = "Your first LP position, made simple" (original)
// ?v=2 = "LP is the New Trading" (new flow)
    ...
    <div className="relative rounded-2xl overflow-hidden border border-white/10 my-12">
      ...
      <div className="relative z-10 grid grid-cols-1 sm:grid-cols-2">
        <div className="p-8 flex flex-col justify-center">
          ...
          <a href="https://lparmy.meteora.ag" target="_blank" rel="noopener noreferrer"
...
function FinalCTA({ headline, sub, note }: { headline: string; sub: string; note: string }) {
  return (
    <div className="relative rounded-2xl overflow-hidden border border-white/10 p-14 text-center my-12">
      ...
      <div className="relative z-10">
        ...
        <p className="text-sm text-white/40 mb-8 max-w-sm mx-auto">{sub}</p>
        ...
        <p className="text-[12px] text-white/25">{note}</p>
      </div>
    </div>
  );
}

// ── Version 1: "Your first LP position, made simple" ─────────────────────────
    ...
    <>
      ...
      <div className="text-center pt-24 pb-16 border-b border-white/10">
        ...
        <div className="relative">
          ...
          <div style={{ aspectRatio: "16/7" }} className="w-full rounded-2xl border border-dashed border-white/20 bg-white/[0.02] flex items-center justify-center">
            <p className="text-[11px] text-white/25 font-medium uppercase tracking-widest">Hero image placeholder</p>
          </div>
        </div>
      </div>
          ...
          {[
            ...
            <div key={c.icon} className="bg-white/[0.03] border border-white/10 rounded-2xl p-6 hover:border-[#F97316]/20 transition-colors">
              <div className="w-12 h-12 rounded-xl bg-[#F97316]/10 border border-[#F97316]/10 flex items-center justify-center mb-5">
                {c.icon === "deposit" && <svg width="24" height="24" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="9" stroke="#F97316" strokeWidth="1.5"/><path d="M12 8v8M9 13l3 3 3-3" stroke="#F97316" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/><path d="M8 8h8" stroke="#F97316" strokeWidth="1.5" strokeLinecap="round" opacity="0.4"/></svg>}
                {c.icon === "fees" && <svg width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M3 17l4-5 4 3 4-6 4 4" stroke="#F97316" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/><path d="M3 12V7a1 1 0 011-1h16a1 1 0 011 1v10a1 1 0 01-1 1H4a1 1 0 01-1-1v-2" stroke="#F97316" strokeWidth="1.5" strokeLinecap="round" opacity="0.4"/></svg>}
                {c.icon === "range" && <svg width="24" height="24" viewBox="0 0 24 24" fill="none"><rect x="3" y="10" width="18" height="4" rx="2" fill="#F97316" fillOpacity="0.15" stroke="#F97316" strokeWidth="1.5" strokeOpacity="0.4"/><path d="M12 3v4M12 17v4" stroke="#F97316" strokeWidth="1.5" strokeLinecap="round" opacity="0.4"/><circle cx="12" cy="12" r="2.5" fill="#F97316"/><path d="M7 12H3M21 12h-4" stroke="#F97316" strokeWidth="1.5" strokeLinecap="round" opacity="0.4"/></svg>}
              </div>
              ...
              {c.tag && <span className={`inline-block text-[11px] px-2 py-0.5 rounded-full border ${c.tagStyle}`}>{c.tag}</span>}
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-white/[0.03] border border-white/10 rounded-2xl overflow-hidden my-10">
        <div className="grid grid-cols-1 sm:grid-cols-2">
          <div className="p-8 border-b sm:border-b-0 sm:border-r border-white/10">
            <SectionLabel>Guided setup tour</SectionLabel>
            ...
            <div className="flex flex-col gap-3 mb-8">
              {["Read pool info", "Deposit tokens", "Set range", "Review", "Create position"].map((s, i) => (
                <div key={s} className="flex items-center gap-3">
                  <span className="shrink-0 w-7 h-7 rounded-full bg-[#F97316]/10 text-[#F97316] text-[11px] font-bold flex items-center justify-center border border-[#F97316]/20">{i + 1}</span>
          ...
          <div className="relative min-h-[280px] bg-white/[0.02] flex items-center justify-center p-6">
            <div style={{ aspectRatio: "4/3" }} className="w-full rounded-2xl border border-dashed border-white/20 bg-white/[0.02] flex items-center justify-center">
              <p className="text-[11px] text-white/25 font-medium uppercase tracking-widest">Tour screenshot</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 my-10">
        <div className="bg-white/[0.03] border border-white/10 rounded-2xl p-6">
          <SectionLabel>After you deposit</SectionLabel>
        ...
        <div className="bg-white/[0.03] border border-white/10 rounded-2xl p-6">
          ...
          <div className="flex flex-col gap-3.5">
            {["Start with a small amount", "Use a pair you recognize", "Pick a wider range if unsure", "Check your position daily at first", "First try doesn't need to be perfect"].map((item) => (
      ...
      <div className="my-10 px-5 py-5 bg-white/[0.02] border border-white/10 rounded-xl">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-[13px] text-white/40 mb-1">This guide covers <span className="text-white font-medium">DLMM</span>, Meteora's flagship product.</p>
            <p className="text-[13px] text-white/40 leading-relaxed"><span className="text-white font-medium">DAMM v2</span> is also available. A more hands-off, set-and-forget pool type suited for longer time horizons. No guided setup yet but you can get a feel for it below.</p>
          </div>
          <a href="#" className="shrink-0 inline-flex items-center gap-1.5 text-[12px] text-[#F97316] border border-[#F97316]/20 bg-[#F97316]/10 px-3 py-1.5 rounded-lg hover:bg-[#F97316]/20 transition-colors whitespace-nowrap">
            <svg width="12" height="12" viewBox="0 0 12 12" fill="none"><circle cx="6" cy="6" r="5" stroke="currentColor" strokeWidth="1.2"/><path d="M4.5 4l4 2-4 2V4z" fill="currentColor"/></svg>
            Watch video
          </a>
        </div>
      </div>
      ...
      <LPAcademy />
      <FinalCTA headline="Ready to start?" sub="Takes 5 minutes. Guided the whole way." note="You do not need to get it perfect on the first try." />
    </>
  );
}

// ── Version 2: "LP is the New Trading" ───────────────────────────────────────
    ...
    <>
      ...
      <div className="text-center pt-24 pb-20 border-b border-white/10">
        ...
        <h1 className="text-4xl sm:text-6xl font-bold text-white leading-[1.1] tracking-tight mb-6">
          LP is the<br /><span className="text-[#F97316]">New Trading</span>
        </h1>
        <div className="relative w-full aspect-video rounded-2xl border border-white/10 bg-white/[0.02] flex items-center justify-center mb-10">
          <div className="w-14 h-14 rounded-full bg-[#F97316]/10 border border-[#F97316]/20 flex items-center justify-center">
            <svg width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M7 5l9 5-9 5V5z" fill="#F97316"/></svg>
          </div>
          <p className="absolute bottom-5 left-0 right-0 text-center text-[12px] text-white/30">Watch this quick intro to see why more traders are starting with LPs on Meteora.</p>
        </div>
        <p className="text-lg text-white/50 max-w-lg mx-auto mb-3 leading-relaxed">If you already trade, LPs give you another way to stay in the market and earn fees without needing to master everything on day one.</p>
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          ...
          <button className="flex items-center justify-center gap-2 border border-white/20 text-white/60 text-sm font-medium px-8 py-3 rounded-lg hover:bg-white/5 hover:text-white transition-colors">
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><circle cx="7" cy="7" r="6" stroke="currentColor" strokeWidth="1.2"/><path d="M5.5 5l4 2-4 2V5z" fill="currentColor"/></svg>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
          <div className="bg-[#F97316]/5 border border-[#F97316]/20 rounded-2xl p-6">
            ...
            <div className="flex flex-col gap-3">
              {["Already trade on chain", "Often hold tokens for a while", "Want more ways to make money than just price moves", "Are open to starting small and learning by doing", "Want to try a new strategy without going all in"].map((item) => (
                <div key={item} className="flex items-start gap-3">
                  <span className="shrink-0 w-[18px] h-[18px] rounded-full bg-[#F97316]/10 border border-[#F97316]/20 flex items-center justify-center mt-0.5"><IconCheck /></span>
          ...
          <div className="bg-white/[0.02] border border-white/10 rounded-2xl p-6">
            ...
            <div className="flex flex-col gap-3">
              {["Only want very fast in and out trades", "Do not want to check on a position at all", "Want every strategy to feel familiar right away", "Do not want to experiment with a small amount first"].map((item) => (
                <div key={item} className="flex items-start gap-3">
                  <span className="shrink-0 w-[18px] h-[18px] rounded-full bg-white/5 border border-white/10 flex items-center justify-center mt-0.5"><IconX /></span>
      ...
      <LPAcademy />
      <FinalCTA headline="Ready to try your first LP?" sub="Start small, keep it simple, and see how LPing feels for yourself." note="You do not need to get it perfect on the first try." />
    </>
  );
}
...
export default function StarterPage({ searchParams }: { searchParams: { v?: string } }) {
  const version = searchParams?.v === "2" ? 2 : 1;
  ...
  return (
    <div className="min-h-screen bg-[#0C0E12] text-white">
      ...
      <Nav />
      {/* Version toggle pill */}
      <div className="fixed bottom-5 left-1/2 -translate-x-1/2 z-50 flex items-center gap-1 bg-[#1a1d23] border border-white/10 rounded-full px-2 py-1.5 shadow-lg">
        <a href="/starter?v=1" className={`text-[11px] font-medium px-3 py-1 rounded-full transition-colors ${version === 1 ? "bg-[#F97316] text-white" : "text-white/40 hover:text-white"}`}>
          v1 — Made simple
        </a>
        <a href="/starter?v=2" className={`text-[11px] font-medium px-3 py-1 rounded-full transition-colors ${version === 2 ? "bg-[#F97316] text-white" : "text-white/40 hover:text-white"}`}>
          v2 — New Trading
        </a>
      </div>
      <main className="relative max-w-4xl mx-auto px-5 pb-24">
        {version === 1 ? <VersionOne /> : <VersionTwo />}
      </main>
    </div>
  );
}